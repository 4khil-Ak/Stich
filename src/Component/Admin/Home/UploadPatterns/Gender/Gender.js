...
// this will enable the book like styling
...
const Gender = (props) => {
  ...
  useEffect(() => {
    ...
    db.collection("gender")
      .where("gender_id", ">=", 1)
      // .where("hide", "==", false) // end-user side can use this
  ...
  const changeNameHandler = (genderName) => {
    console.log("gender name updated", genderName);
  };
  const changeImageHandler = (genderName) => {
    // db.collection("gender")
    //   .doc(genderName)
    //   .update({
    ...
    //   });
    console.log("gender image updated", genderName);
  };
  ...
  const changeSubmitHandler = () => {
    ...
    if (newData.img !== null && newData.name === "") {
      console.log(genderList.genderName, newData.img);
    } else {
      console.log(genderList.genderName, newData.name);
    }
    setNewData({
      name: "",
      img: null
